...
func _ready() -> void:
	...
	GameAPI.feed_updated.connect(_on_feed)
	GameAPI.fetch_feed(40)


func _exit_tree() -> void:
	if GameAPI.feed_updated.is_connected(_on_feed):
		GameAPI.feed_updated.disconnect(_on_feed)


func _on_feed(entries: Array) -> void:
	...
	if entries.is_empty():
		var lbl := Label.new()
		lbl.text = "No activity yet — start working to fill this feed!"
...
func _make_row(entry: Dictionary) -> Control:
	...
	desc_lbl.modulate = _EVENT_COLOR.get(etype, Color.WHITE)
